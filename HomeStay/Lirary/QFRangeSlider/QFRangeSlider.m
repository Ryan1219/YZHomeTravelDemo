...
@property (nonatomic,assign) NSInteger showUnit;
...
//MARK:--QFRangeSlider implementation
@implementation QFRangeSlider
...
- (void)initialiseControl {
    ...
    _handleDiameter = 30.0; //左右移动的滑动条大小
    _selectedHandleDiameterMultiplier = 1.0; //决定滑动动画时缩放的程度,即滑动时是否缩放
    _lineHeight = 4.0; //中间线条高度
    ...
    self.sliderLine.backgroundColor = [UIColor whiteColor].CGColor; //背景色
    self.sliderLine.borderWidth = 0.5;
    self.sliderLine.borderColor = Color_5aa572.CGColor;
...
//    self.sliderLineBetweenHandles.backgroundColor = self.tintColor.CGColor;
    self.sliderLineBetweenHandles.backgroundColor = Color_5aa572.CGColor; //背景色
    ...
    self.leftHandle.masksToBounds = true;
...
//    self.leftHandle.borderColor = self.handleBorderColor.CGColor;
    ...
    self.rightHandle.masksToBounds = true;
...
//MARK:--layoutSubviews 默认线条的frame在这设置
- (void)layoutSubviews {
...
//    if (self.showUnit == 0) { //add 20170913
...
//    } else {
//        NSString *minString = [NSString stringWithFormat:@"%@万",[formatter stringFromNumber:@(self.selectedMinimum)]];
//        self.minLabel.string = minString;
//        
//        NSString *maxString = [NSString stringWithFormat:@"%@万",[formatter stringFromNumber:@(self.selectedMaximum)]];
...
- (void)setIsAddUnit:(NSInteger)isAddUnit {

    self.showUnit = isAddUnit;
    [self updateLabelValues];
...
//MARK:--TTRangeSliderLeftElement implementation
@implementation TTRangeSliderLeftElement

- (void)accessibilityIncrement {
    QFRangeSlider* slider = (QFRangeSlider*)self.accessibilityContainer;
...
//MARK:--TTRangeSliderRightElement implementation
